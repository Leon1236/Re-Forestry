...
public final class GuiPowerLedger {
	...
	private final IContainerEnergy energy;
	...
	public GuiPowerLedger(IContainerEnergy energy) {
		this.energy = energy;
	}
	...
	public void draw(GuiGraphicsExtractor graphics, Font font, int leftPos, int topPos, int imageWidth, int mouseX, int mouseY) {
		...
		if (width < MAX_WIDTH - 1) {
			if (isOverTab(mouseX, mouseY) && !this.open) {
				graphics.setTooltipForNextFrame(font,
						List.of(Component.literal(formatEnergy(this.energy.getEnergyStored()))),
		...
		graphics.text(font, Component.translatable("for.gui.stored").append(":"), xBody, this.ledgerY + 20, LABEL_COLOR, false);
		graphics.text(font, Component.literal(formatEnergy(this.energy.getEnergyStored())), xBody, this.ledgerY + 32, VALUE_COLOR, false);

		graphics.text(font, Component.translatable("for.gui.energyusage").append(":"), xBody, this.ledgerY + 44, LABEL_COLOR, false);
		graphics.text(font, Component.literal(formatRate(this.energy.getEnergyUsage())), xBody, this.ledgerY + 56, VALUE_COLOR, false);

		graphics.text(font, Component.translatable("for.gui.maxenergy").append(":"), xBody, this.ledgerY + 68, LABEL_COLOR, false);
		graphics.text(font, Component.literal(formatEnergy(this.energy.getEnergyCapacity())), xBody, this.ledgerY + 80, VALUE_COLOR, false);

		graphics.text(font, Component.translatable("for.gui.maxenergyreceive").append(":"), xBody, this.ledgerY + 92, LABEL_COLOR, false);
		graphics.text(font, Component.literal(formatEnergy(this.energy.getEnergyMaxReceive())), xBody, this.ledgerY + 104, VALUE_COLOR, false);
	}
