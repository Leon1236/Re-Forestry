...
public final class ClimateStateHelper implements IClimateStateHelper {
	...
	public IClimateState create(CompoundTag compound, boolean mutable) {
		if (compound.getBooleanOr(ABSENT_NBT_KEY, false)) {
			return AbsentClimateState.INSTANCE;
		}
		return checkState(create(compound.getFloatOr(TEMPERATURE_NBT_KEY, 0.0F), compound.getFloatOr(HUMIDITY_NBT_KEY, 0.0F), mutable));
	}

	@Override
	public IClimateState create(CompoundTag compound) {
		return create(compound, compound.getBooleanOr(MUTABLE_NBT_KEY, false));
	}
