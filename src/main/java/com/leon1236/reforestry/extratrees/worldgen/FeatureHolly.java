...
public class FeatureHolly extends FeatureBinnieTree {
	public FeatureHolly(ITreeGenData tree) {
		super(tree, 4, 2);
	}
	...
	protected void generateLeaves(IGenome genome, LevelAccessor level, RandomSource rand, TreeBlockTypeLeaf leaf, TreeContour contour, BlockPos startPos) {
		...
		float width = this.height * randBetween(rand, 0.4f, 0.45f);
		if (width < 1.5f) {
			width = 1.5f;
		}
		...
		generateCylinder(level, rand, leaf, contour, startPos, 0.0f, leafSpawn, 0.0f, 1.0f);
	}
}
