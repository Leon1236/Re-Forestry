...
public class FeatureCopperBeech extends FeatureBinnieTree {
	public FeatureCopperBeech(ITreeGenData tree) {
		super(tree, 5, 3);
	}
	...
	protected void generateLeaves(IGenome genome, LevelAccessor level, RandomSource rand, TreeBlockTypeLeaf leaf, TreeContour contour, BlockPos startPos) {
		...
		while(leafSpawn > bottom) {
			generateCylinder(level, rand, leaf, contour, startPos, 0, leafSpawn--, 0, this.girth + 2.5f);
			generateCylinder(level, rand, leaf, contour, startPos, 0, leafSpawn--, 0, this.girth + 1.85f);
		}
		generateCylinder(level, rand, leaf, contour, startPos, 0, leafSpawn--, 0, this.girth + 1.5f);
	}
}
