...
import com.leon1236.reforestry.gendustry.item.IGendustryUpgradeType;
...
class IndustrialApiaryBeeModifier implements IBeeModifier {
	...
	public int recalculate(Container inventory) {
		reset();

		int energyCost = 0;

		for (int i = 0; i < IndustrialApiaryBlockEntity.UPGRADE_SLOT_COUNT; i++) {
			ItemStack stack = inventory.getItem(IndustrialApiaryBlockEntity.UPGRADE_SLOT_START + i);
	...
	public Vec3i modifyTerritory(IGenome genome, Vec3i currentModifier) {
		return new Vec3i(
				(int) (currentModifier.getX() * this.territory),
				(int) (currentModifier.getY() * this.territory),
				(int) (currentModifier.getZ() * this.territory));
	}
