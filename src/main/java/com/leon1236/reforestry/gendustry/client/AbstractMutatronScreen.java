...
import com.leon1236.reforestry.gendustry.menu.AbstractMutatronMenu;

public class AbstractMutatronScreen<M extends AbstractMutatronMenu<?>> extends ScreenForestry<M> {
	...
	public AbstractMutatronScreen(Identifier texture, M menu, Inventory inventory, Component title) {
	...
	public void extractBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta) {
		...
		GendustryGuiTextures.blitBackground(guiGraphics, this.texture, leftPos, topPos, imageWidth, imageHeight);

		drawTank(guiGraphics, mouseX, mouseY);

		int progress = menu.getProgressPercent() * 55 / 100;
		GendustryGuiTextures.blitProgress(guiGraphics, this.texture,
				leftPos + 68, topPos + 38, 176, 60, progress, 18);

		GuiErrorTabs.draw(guiGraphics, font, leftPos, topPos, menu::getErrorCount, menu::getErrorId,
				AbstractMutatronBlockEntity.ERROR_SLOT_COUNT, mouseX, mouseY);
	}
	...
	private void drawTank(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
		...
		if (mouseX >= tankLeft && mouseX < tankLeft + TANK_WIDTH && mouseY >= tankTop && mouseY < tankTop + TANK_HEIGHT) {
			Fluid fluid = menu.getMutagenFluid();
			Component fluidName;
