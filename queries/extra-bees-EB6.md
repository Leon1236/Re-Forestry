...
Amounts: drops **200** mB / time **10**; propolis **500** mB / time **20** (Binnie). Dye-drop remnants use Extra Bees `dye_*` or modern `minecraft:*_dye` at chance **1.0**.
...
Partial centrifuge recipes still ship when other products resolve (e.g. uranium/stone wax+honey; sawdust keeps honey_drop).
...
3. Squeeze `propolis_water` → water
4. `honey_drop_acid` / `propolis_oil` have **no** squeezer recipe (soft-skip)
