...
Last updated: 2026-08-21 (GD3: mutagen producer / protein liquefier / DNA extractor. Next: GD4)
...
**Next up: Wave 7 `GD4`.** Gendustry GD3 producer machines landed (mutagen producer, protein liquefier, DNA extractor + shared processor GUI). Locked remaining order: see `queries/wave7-plan.md` (GD3 → GD4…). No solar/combustion.

**Wave 7 addons:** GD0 + EB1 + GD1 + GD2 + GD3 done; Extra Trees still extract-only until ET1a.

| ID | Status | Notes |
|---|---|---|
| `GD0` | Done | `ModuleGendustry` + plugin; depends arboriculture; 10 resource parts + 17 upgrades + 6 elite + `pollen_kit`; tag `reforestry:upgrades`; crafts; `IPollen.createStack` via `TreePollenType`. Upgrades inert until GD7b. See `queries/gendustry-GD0.md` |
...
| `GD3` | Done | `mutagen_producer` / `protein_liquefier` / `dna_extractor`; shared `processor` menu/screen; FE ctor swap (1M/10k); labware 10% consume; crafts/loot/tags/hints. See `queries/gendustry-GD3.md` |
...
| 3 | Addons | Gendustry → Extra Bees → Extra Trees | Config modules; **GD0 + EB1 + GD1 + GD2 + GD3 done**. Next: **GD4** |
