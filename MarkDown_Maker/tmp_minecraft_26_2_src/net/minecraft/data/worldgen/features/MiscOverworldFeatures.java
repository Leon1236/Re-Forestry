			...
			new CompositeFeatureConfiguration(
				HolderSet.direct(
					...
					PlacementUtils.inlinePlaced(
						...
						new SimpleBlockConfiguration(BlockStateProvider.simple(Blocks.POTENT_SULFUR.defaultBlockState().setValue(PotentSulfurBlock.STATE, PotentSulfurState.WET))),
						EnvironmentScanPlacement.scanningFor(
							Direction.DOWN, BlockPredicate.allOf(BlockPredicate.solid(), BlockPredicate.matchesFluids(Direction.UP.getUnitVec3i(), Fluids.WATER)), 4
						)
					)
				)
			)
						...
						PlacementUtils.inlinePlaced(
							...
							new CompositeFeatureConfiguration(
								HolderSet.direct(
									...
									sulfurSprings.apply(
										WeightedList.of(TemplateFeatureConfiguration.TemplateEntry.of(Identifier.withDefaultNamespace("spring/sulfur_spring_extra_large_1")))
									)
								)
							)
						),
		...
		FeatureUtils.register(
			...
			new DiskConfiguration(
				new RuleBasedStateProvider(
					...
					List.of(
						new RuleBasedStateProvider.Rule(BlockPredicate.matchesBlocks(Direction.DOWN.getUnitVec3i(), Blocks.AIR), BlockStateProvider.simple(Blocks.SANDSTONE))
					)
				),
