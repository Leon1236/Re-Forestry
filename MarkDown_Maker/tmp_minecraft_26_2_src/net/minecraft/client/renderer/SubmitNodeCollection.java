...
public class SubmitNodeCollection implements OrderedSubmitNodeCollector {
	...
	public <S> void submitModel(
		...
		if (outlineColor != 0) {
			...
			if (outlineRenderType != null) {
				this.outline
					.submit(new ModelFeatureRenderer.Submit<>(outlineRenderType, pose, model, state, 15728880, OverlayTexture.NO_OVERLAY, outlineColor, sprite, null));
			}
		}
		...
		if (crumblingOverlay != null && renderType.affectsCrumbling()) {
			...
			this.breakingOverlay
				.submit(
					new ModelFeatureRenderer.Submit<>(crumblingRenderType, pose, model, state, lightCoords, overlayCoords, tintedColor, null, crumblingOverlay.cameraPose())
				);
		}
	}
	...
	public void submitBreakingBlockModel(final PoseStack poseStack, final List<BlockStateModelPart> parts, final int progress) {
		...
		this.breakingOverlay
			.submit(
				new BlockModelFeatureRenderer.Submit(
					pose, ModelBakery.DESTROY_TYPES.get(progress), List.copyOf(parts), BlockModelRenderState.EMPTY_TINTS, 15728880, OverlayTexture.NO_OVERLAY, 0, pose
				)
			);
	}
