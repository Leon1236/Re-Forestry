...
public class ObjectiveCriteria {
	...
	public static final Codec<ObjectiveCriteria> CODEC = Codec.STRING
		.comapFlatMap(
			name -> byName(name).map(DataResult::success).orElse(DataResult.error(() -> "No scoreboard criteria with name: " + name)), ObjectiveCriteria::getName
		);
	 ...
	 * Access widened by fabric-transitive-access-wideners-v1 to accessible
	 */
	public static ObjectiveCriteria registerCustom(final String name, final boolean readOnly, final ObjectiveCriteria.RenderType renderType) {
	 ...
	 * Access widened by fabric-transitive-access-wideners-v1 to accessible
	 */
	public static ObjectiveCriteria registerCustom(final String name) {
