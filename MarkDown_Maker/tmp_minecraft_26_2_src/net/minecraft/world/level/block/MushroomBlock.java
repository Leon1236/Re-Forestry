...
public class MushroomBlock extends VegetationBlock implements BonemealableBlock {
	public static final MapCodec<MushroomBlock> CODEC = RecordCodecBuilder.mapCodec(
		i -> i.group(ResourceKey.codec(Registries.CONFIGURED_FEATURE).fieldOf("feature").forGetter(b -> b.feature), propertiesCodec()).apply(i, MushroomBlock::new)
	);
	...
	public boolean isValidBonemealTarget(final LevelReader level, final BlockPos pos, final BlockState state) {
		if (level instanceof ServerLevel serverLevel) {
			...
			if (featureHolder.isPresent()) {
				...
				if (configuredFeature.feature() instanceof AbstractHugeMushroomFeature && configuredFeature.config() instanceof HugeMushroomFeatureConfiguration config) {
