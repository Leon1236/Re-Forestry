...
public class EntityPositionSource implements PositionSource {
	public static final MapCodec<EntityPositionSource> CODEC = RecordCodecBuilder.mapCodec(
		i -> i.group(
				UUIDUtil.CODEC.fieldOf("source_entity").forGetter(EntityPositionSource::getUuid), Codec.FLOAT.optionalFieldOf("y_offset", 0.0F).forGetter(o -> o.yOffset)
			)
	...
	private void resolveEntity(final Level level) {
		this.entityOrUuidOrId
			.map(
				Optional::of,
				uuidOrId -> Optional.ofNullable(uuidOrId.map(uuid -> level instanceof ServerLevel serverLevel ? serverLevel.getEntity(uuid) : null, level::getEntity))
			)
