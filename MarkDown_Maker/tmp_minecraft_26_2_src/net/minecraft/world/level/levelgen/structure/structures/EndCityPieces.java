...
public class EndCityPieces {
	...
	private static final EndCityPieces.SectionGenerator TOWER_GENERATOR = new EndCityPieces.SectionGenerator() {
		...
		public boolean generate(
			...
			lastPiece = EndCityPieces.addHelper(
				pieces,
				EndCityPieces.addPiece(structureTemplateManager, lastPiece, new BlockPos(3 + random.nextInt(2), -3, 3 + random.nextInt(2)), "tower_base", rotation, true)
			);
			lastPiece = EndCityPieces.addHelper(
			...
			if (bridgePiece != null) {
				for (Pair<Rotation, BlockPos> bridge : EndCityPieces.TOWER_BRIDGES) {
					if (random.nextBoolean()) {
						EndCityPieces.EndCityPiece bridgeStart = EndCityPieces.addHelper(
							pieces, EndCityPieces.addPiece(structureTemplateManager, bridgePiece, bridge.getSecond(), "bridge_end", rotation.getRotated(bridge.getFirst()), true)
						);
						EndCityPieces.recursiveChildren(structureTemplateManager, EndCityPieces.TOWER_BRIDGE_GENERATOR, genDepth + 1, bridgeStart, null, pieces, random);
					}
				}
				...
				if (genDepth != 7) {
					return EndCityPieces.recursiveChildren(structureTemplateManager, EndCityPieces.FAT_TOWER_GENERATOR, genDepth + 1, lastPiece, null, pieces, random);
				}
		...
		public boolean generate(
			...
			if (!this.shipCreated && random.nextInt(10 - genDepth) == 0) {
				EndCityPieces.addHelper(
					pieces,
					EndCityPieces.addPiece(structureTemplateManager, lastPiece, new BlockPos(-8 + random.nextInt(8), nextY, -70 + random.nextInt(10)), "ship", rotation, true)
				);
			...
			lastPiece = EndCityPieces.addHelper(
				pieces,
				EndCityPieces.addPiece(structureTemplateManager, lastPiece, new BlockPos(4, nextY, 0), "bridge_end", rotation.getRotated(Rotation.CLOCKWISE_180), true)
			);
			lastPiece.setGenDepth(-1);
			return true;
		}
		...
		public boolean generate(
			...
			for (int i = 0; i < 2 && random.nextInt(3) != 0; i++) {
				...
				for (Pair<Rotation, BlockPos> bridge : EndCityPieces.FAT_TOWER_BRIDGES) {
					if (random.nextBoolean()) {
						EndCityPieces.EndCityPiece bridgeStart = EndCityPieces.addHelper(
							pieces, EndCityPieces.addPiece(structureTemplateManager, lastPiece, bridge.getSecond(), "bridge_end", rotation.getRotated(bridge.getFirst()), true)
						);
						EndCityPieces.recursiveChildren(structureTemplateManager, EndCityPieces.TOWER_BRIDGE_GENERATOR, genDepth + 1, bridgeStart, null, pieces, random);
					}
				}
	...
	public static class EndCityPiece extends TemplateStructurePiece {
		public EndCityPiece(
			...
			super(
				StructurePieceType.END_CITY_PIECE, 0, structureTemplateManager, makeIdentifier(templateName), templateName, makeSettings(overwrite, rotation), position
			);
		}
