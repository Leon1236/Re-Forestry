...
public class JigsawPlacement {
	...
	public static Optional<Structure.GenerationStub> addPieces(
		...
		if (startJigsaw.isPresent()) {
			...
			if (anchor.isEmpty()) {
				LOGGER.error(
					"No starting jigsaw {} found in start pool {}", targetJigsawId, startPool.unwrapKey().map(key -> key.identifier().toString()).orElse("<unregistered>")
				);
				return Optional.empty();
			}
								...
								for (Rotation targetRotation : Rotation.getShuffled(this.random)) {
									...
									for (StructureTemplate.JigsawBlockInfo targetJigsaw : targetJigsaws) {
										if (JigsawBlock.canAttach(sourceJigsaw, targetJigsaw)) {
											...
											if (sourceRigid && targetRigid) {
												...
												if (sourceJigsawBaseHeight == Integer.MIN_VALUE) {
													sourceJigsawBaseHeight = this.chunkGenerator
														.getFirstFreeHeight(sourceJigsawPos.getX(), sourceJigsawPos.getZ(), Heightmap.Types.WORLD_SURFACE_WG, heightAccessor, randomState);
												}
											...
											if (!Shapes.joinIsNotEmpty(childrenFree.get(), Shapes.create(AABB.of(targetBB).deflate(0.25)), BooleanOp.ONLY_SECOND)) {
												...
												PoolElementStructurePiece targetPiece = new PoolElementStructurePiece(
													this.structureTemplateManager, targetElement, targetBoxPosition, targetGroundLevelDelta, targetRotation, targetBB, liquidSettings
												);
												int junctionY;
												if (sourceRigid) {
												...
												sourcePiece.addJunction(
													new JigsawJunction(
														targetJigsawPos.getX(), junctionY - sourceJigsawLocalY + sourceGroundLevelDelta, targetJigsawPos.getZ(), deltaY, targetProjection
													)
												);
												targetPiece.addJunction(
													new JigsawJunction(
														sourceJigsawPos.getX(), junctionY - targetJigsawLocalY + targetGroundLevelDelta, sourceJigsawPos.getZ(), -deltaY, sourceProjection
													)
												);
