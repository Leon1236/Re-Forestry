	...
	public static BehaviorControl<LivingEntity> create() {
		...
		return BehaviorBuilder.create(
			i -> i.group(i.present(MemoryModuleType.PATH), i.registered(MemoryModuleType.DOORS_TO_CLOSE), i.registered(MemoryModuleType.NEAREST_LIVING_ENTITIES))
				.apply(i, (pathMemory, doorsMemory, nearestEntities) -> (level, body, timestamp) -> {
					...
					if (!path.notStarted() && !path.isDone()) {
						...
						doors.ifPresent(doorSet -> closeDoorsThatIHaveOpenedOrPassedThrough(level, body, fromNode, toNode, (Set<GlobalPos>)doorSet, i.tryGet(nearestEntities)));
						return true;
					} else {
						return false;
					}
