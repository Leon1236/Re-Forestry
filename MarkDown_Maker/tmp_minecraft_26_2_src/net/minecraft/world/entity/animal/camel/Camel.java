...
public class Camel extends AbstractHorse {
	...
	private static final EntityDimensions ADULT_SITTING_DIMENSIONS = EntityDimensions.scalable(EntityTypes.CAMEL.getWidth(), EntityTypes.CAMEL.getHeight() - 1.43F)
	...
	protected boolean handleEating(final Player player, final ItemStack itemStack) {
		...
		if (!this.isSilent()) {
			...
			if (eatingSound != null) {
				this.level()
					.playSound(
						null, this.getX(), this.getY(), this.getZ(), eatingSound, this.getSoundSource(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F
					);
			}
		}
