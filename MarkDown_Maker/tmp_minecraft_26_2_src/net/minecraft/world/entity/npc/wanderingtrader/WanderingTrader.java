...
public class WanderingTrader extends AbstractVillager implements Consumable.OverrideConsumeSound {
	...
	protected void registerGoals() {
		...
		this.goalSelector
			.addGoal(
				0,
				new UseItemGoal<>(this, new ItemStack(Items.MILK_BUCKET), SoundEvents.WANDERING_TRADER_REAPPEARED, e -> this.level().isBrightOutside() && e.isInvisible())
			);
		...
		public void tick() {
			...
			if (wanderPosition != null && WanderingTrader.this.navigation.isDone()) {
				if (this.isTooFarAway(wanderPosition, 10.0)) {
					Vec3 dir = new Vec3(wanderPosition.getX() - this.trader.getX(), wanderPosition.getY() - this.trader.getY(), wanderPosition.getZ() - this.trader.getZ())
						.normalize();
