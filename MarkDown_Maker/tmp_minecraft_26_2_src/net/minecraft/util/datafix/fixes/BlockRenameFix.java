...
public abstract class BlockRenameFix extends DataFix {
	...
	public TypeRewriteRule makeRule() {
		...
		TypeRewriteRule blockStateRule = this.fixTypeEverywhereTyped(
			this.name + " for block_state", this.getInputSchema().getType(References.BLOCK_STATE), input -> input.update(DSL.remainderFinder(), this::fixBlockState)
		);
		TypeRewriteRule flatBlockStateRule = this.fixTypeEverywhereTyped(
			...
			input -> input.update(DSL.remainderFinder(), tag -> DataFixUtils.orElse(tag.asString().result().map(this::fixFlatBlockState).map(tag::createString), tag))
		);
