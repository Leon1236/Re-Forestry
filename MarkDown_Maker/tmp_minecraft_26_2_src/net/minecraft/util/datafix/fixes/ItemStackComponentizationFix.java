...
public class ItemStackComponentizationFix extends DataFix {
		...
		return switch (id) {
			...
			case "minecraft:decorated_pot" -> {
				...
				if (item.isPresent()) {
					itemStack.setComponent(
						"minecraft:container", blockEntity.createList(Stream.of(blockEntity.emptyMap().set("slot", blockEntity.createInt(0)).set("item", item.get())))
					);
				}
			...
			case "minecraft:shulker_box", "minecraft:chest", "minecraft:trapped_chest", "minecraft:furnace", "minecraft:ender_chest", "minecraft:dispenser", "minecraft:dropper", "minecraft:brewing_stand", "minecraft:hopper", "minecraft:barrel", "minecraft:smoker", "minecraft:blast_furnace", "minecraft:campfire", "minecraft:chiseled_bookshelf", "minecraft:crafter" -> {
				List<Dynamic<T>> items = blockEntity.get("Items")
					.asList(dynamic -> dynamic.emptyMap().set("slot", dynamic.createInt(dynamic.get("Slot").asByte((byte)0) & 255)).set("item", dynamic.remove("Slot")));
				if (!items.isEmpty()) {
	...
	private static void fixBlockStatePredicates(
		final ItemStackComponentizationFix.ItemStackData itemStack, final Dynamic<?> dynamic, final String tag, final String componentId, final boolean hideInTooltip
	) {
			...
			Dynamic<?> component = dynamic.emptyMap()
				.set(
					...
					dynamic.createList(
						oldPredicate.get()
							.asStream()
							.map(value -> DataFixUtils.orElse(value.asString().map(string -> fixBlockStatePredicate((Dynamic<?>)value, string)).result(), value))
					)
				);
			if (hideInTooltip) {
				component = component.set("show_in_tooltip", dynamic.createBoolean(false));
	...
	private static void fixFireworkRocket(final ItemStackComponentizationFix.ItemStackData itemStack) {
		itemStack.fixSubTag(
			...
			fireworks -> {
				...
				itemStack.setComponent(
					"minecraft:fireworks", fireworks.emptyMap().set("explosions", fireworks.createList(explosions)).set("flight_duration", fireworks.createByte((byte)flight))
				);
