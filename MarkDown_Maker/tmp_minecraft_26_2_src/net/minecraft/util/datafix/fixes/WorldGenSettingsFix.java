...
public class WorldGenSettingsFix extends DataFix {
									...
									flatSettings.get("layers")
										...
										.orElseGet(
											() -> input.createList(
												Stream.of(
													input.createMap(
														ImmutableMap.of(input.createString("height"), input.createInt(1), input.createString("block"), input.createString("minecraft:bedrock"))
													),
													input.createMap(
														ImmutableMap.of(input.createString("height"), input.createInt(2), input.createString("block"), input.createString("minecraft:dirt"))
													),
													input.createMap(
														ImmutableMap.of(input.createString("height"), input.createInt(1), input.createString("block"), input.createString("minecraft:grass_block"))
													)
												)
											)
										),
	...
	protected static <T> T vanillaLevels(final Dynamic<T> input, final long seed, final Dynamic<T> overworldGenerator, final boolean caves) {
		...
		return ops.createMap(
			ImmutableMap.of(
				...
				ops.createMap(
					ImmutableMap.of(
						ops.createString("type"), ops.createString("minecraft:overworld" + (caves ? "_caves" : "")), ops.createString("generator"), overworldGenerator.getValue()
					)
				),
				...
				ops.createMap(
					ImmutableMap.of(
						...
						noise(
								...
								input.createMap(
									ImmutableMap.of(input.createString("type"), input.createString("minecraft:the_end"), input.createString("seed"), input.createLong(seed))
								)
							)
							.getValue()
					)
	...
	private static <T> Map<Dynamic<T>, Dynamic<T>> fixFlatStructures(final DynamicOps<T> ops, final OptionalDynamic<T> settings) {
		...
		settings.get("structures")
			...
			.ifSuccess(map -> map.forEach((structureKey, value1) -> value1.getMapValues().result().ifPresent(m -> m.forEach((optionKey, optionValue) -> {
					...
					switch (optionName) {
						...
						case "separation":
							if ("oceanmonument".equals(structureName)) {
								WorldGenSettingsFix.StructureFeatureConfiguration config = structureConfig.getOrDefault("minecraft:monument", DEFAULTS.get("minecraft:monument"));
								int spacing = getInt(value, config.separation, 1);
								structureConfig.put("minecraft:monument", new WorldGenSettingsFix.StructureFeatureConfiguration(spacing, config.separation, config.salt));
							}
		...
		structureBuilder.put(
			...
			settings.createMap(structureConfig.entrySet().stream().collect(Collectors.toMap(e -> settings.createString(e.getKey()), e -> e.getValue().serialize(ops))))
		);
