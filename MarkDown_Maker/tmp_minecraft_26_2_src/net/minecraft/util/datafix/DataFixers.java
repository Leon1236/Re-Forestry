...
public class DataFixers {
	...
	private static void addFixers(final DataFixerBuilder fixerUpper, final FileFixerUpper.Builder fileFixerUpper) {
					...
					return this.fixTypeEverywhereTyped(
						...
						input -> {
							...
							return !"minecraft:commandblock_minecart".equals(id)
								? input
								: input.updateTyped(
									customNameFinder, customName -> customName.update(componentFinder, pair -> pair.mapSecond(LegacyComponentDataFixUtils::createTextComponentJson))
								);
						}
					);
				}
		...
		ImmutableMap<String, String> catAdvancementMigrationMap = ImmutableMap.<String, String>builder()
			...
			.build();
		fixerUpper.addFixer(
			new CriteriaRenameFix(v3086, "Migrate cat variant advancement", "minecraft:husbandry/complete_catalogue", s -> catAdvancementMigrationMap.getOrDefault(s, s))
		);
		...
		fixerUpper.addFixer(
			new CriteriaRenameFix(
				v3097, "Migrate cat variant advancement for british shorthair", "minecraft:husbandry/complete_catalogue", s -> renamedCatCriteria.getOrDefault(s, s)
			)
		);
		...
		fixerUpper.addFixer(
			BlockEntityRenameFix.create(v3438, "Rename Suspicious Sand to Brushable Block", createRenamer("minecraft:suspicious_sand", "minecraft:brushable_block"))
		);
		...
		fixerUpper.addFixer(
			new BlockPropertyRenameAndFix(
				v4294, "CreakingHeartBlockStateFix", "minecraft:creaking_heart", "active", "creaking_heart_state", value -> value.equals("true") ? "awake" : "uprooted"
			)
		);
