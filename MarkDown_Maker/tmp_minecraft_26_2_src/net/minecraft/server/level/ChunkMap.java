...
public class ChunkMap extends SimpleRegionStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
	...
	private CompletableFuture<ChunkAccess> scheduleChunkLoad(final ChunkPos pos) {
		...
		return chunkDataFuture.<Object, Optional<SerializableChunkData>>thenCombine((CompletionStage<? extends Object>)poiFuture, (chunkData, ignored) -> chunkData)
	...
	void dumpChunks(final Writer output) throws IOException {
		...
		for (Entry<ChunkHolder> entry : this.visibleChunkMap.long2ObjectEntrySet()) {
			...
			Optional<LevelChunk> fullChunk = chunk.flatMap(chunkAccess -> chunkAccess instanceof LevelChunk levelChunk ? Optional.of(levelChunk) : Optional.empty());
			csvOutput.writeRow(
				pos.x(),
				pos.z(),
	...
	private void updateChunkTracking(final ServerPlayer player) {
		...
		if (!(
			player.getChunkTrackingView() instanceof ChunkTrackingView.Positioned view && view.center().equals(chunkPos) && view.viewDistance() == playerViewDistance
		)) {
