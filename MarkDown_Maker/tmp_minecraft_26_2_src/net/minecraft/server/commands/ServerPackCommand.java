							...
							Commands.argument("url", StringArgumentType.string())
								.then(
									Commands.argument("uuid", UuidArgument.uuid())
										...
										.executes(c -> pushPack(c.getSource(), StringArgumentType.getString(c, "url"), Optional.of(UuidArgument.getUuid(c, "uuid")), Optional.empty()))
								)
								.executes(c -> pushPack(c.getSource(), StringArgumentType.getString(c, "url"), Optional.empty(), Optional.empty()))
						)
				)
				.then(Commands.literal("pop").then(Commands.argument("uuid", UuidArgument.uuid()).executes(c -> popPack(c.getSource(), UuidArgument.getUuid(c, "uuid")))))
		);
	}
