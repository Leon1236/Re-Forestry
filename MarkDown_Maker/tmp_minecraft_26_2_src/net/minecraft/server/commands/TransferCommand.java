			...
			Commands.literal("transfer")
				...
				.then(
					Commands.argument("hostname", StringArgumentType.string())
						.executes(c -> transfer(c.getSource(), StringArgumentType.getString(c, "hostname"), 25565, List.of(c.getSource().getPlayerOrException())))
						.then(
							Commands.argument("port", IntegerArgumentType.integer(1, 65535))
								.executes(
									c -> transfer(
										c.getSource(), StringArgumentType.getString(c, "hostname"), IntegerArgumentType.getInteger(c, "port"), List.of(c.getSource().getPlayerOrException())
									)
								)
								.then(
									Commands.argument("players", EntityArgument.players())
										.executes(
											c -> transfer(
												c.getSource(), StringArgumentType.getString(c, "hostname"), IntegerArgumentType.getInteger(c, "port"), EntityArgument.getPlayers(c, "players")
											)
										)
								)
						)
