...
public class WardenSpawnTrackerCommand {
					...
					Commands.literal("set")
						.then(
							Commands.argument("warning_level", IntegerArgumentType.integer(0, 4))
								.executes(
									c -> setWarningLevel(c.getSource(), ImmutableList.of(c.getSource().getPlayerOrException()), IntegerArgumentType.getInteger(c, "warning_level"))
								)
						)
				)
		);
	...
	private static int resetTracker(final CommandSourceStack source, final Collection<? extends Player> players) {
		...
		if (players.size() == 1) {
			source.sendSuccess(() -> Component.translatable("commands.warden_spawn_tracker.clear.success.single", players.iterator().next().getDisplayName()), true);
		} else {
