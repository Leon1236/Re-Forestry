							...
							Commands.argument("targets", EntityArgument.players())
								.then(
									Commands.argument("dialog", ResourceOrIdArgument.dialog(context))
										.executes(c -> showDialog((CommandSourceStack)c.getSource(), EntityArgument.getPlayers(c, "targets"), ResourceOrIdArgument.getDialog(c, "dialog")))
								)
						)
				)
				.then(
					Commands.literal("clear")
						.then(Commands.argument("targets", EntityArgument.players()).executes(c -> clearDialog(c.getSource(), EntityArgument.getPlayers(c, "targets"))))
				)
		);
	}
