...
public class EffectCommands {
	...
	private static int giveEffect(
		...
		if (entities.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.effect.give.success.single", effect.getDisplayName(), entities.iterator().next().getDisplayName(), duration / 20),
				true
			);
		} else {
	...
	private static int clearEffect(final CommandSourceStack source, final Collection<? extends Entity> entities, final Holder<MobEffect> effectHolder) throws CommandSyntaxException {
		...
		if (entities.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.effect.clear.specific.success.single", effect.getDisplayName(), entities.iterator().next().getDisplayName()), true
			);
		} else {
