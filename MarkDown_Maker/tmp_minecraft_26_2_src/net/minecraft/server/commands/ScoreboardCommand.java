...
public class ScoreboardCommand {
										...
										.then(
											Commands.literal("displayname")
												.then(
													Commands.argument("displayName", ComponentArgument.textComponent(context))
														.executes(
															c -> setDisplayName(c.getSource(), ObjectiveArgument.getObjective(c, "objective"), ComponentArgument.getResolvedComponent(c, "displayName"))
														)
												)
										)
											...
											Commands.literal("displayautoupdate")
												.then(
													Commands.argument("value", BoolArgumentType.bool())
														.executes(c -> setDisplayAutoUpdate(c.getSource(), ObjectiveArgument.getObjective(c, "objective"), BoolArgumentType.getBool(c, "value")))
												)
										)
										.then(
											addNumberFormats(
												...
												(c, numberFormat) -> setObjectiveFormat(c.getSource(), ObjectiveArgument.getObjective(c, "objective"), numberFormat)
											)
										)
								)
						)
							...
							Commands.literal("setdisplay")
								.then(
									Commands.argument("slot", ScoreboardSlotArgument.displaySlot())
										...
										.then(
											Commands.argument("objective", ObjectiveArgument.objective())
												.executes(c -> setDisplaySlot(c.getSource(), ScoreboardSlotArgument.getDisplaySlot(c, "slot"), ObjectiveArgument.getObjective(c, "objective")))
										)
								)
						)
				)
							...
							Commands.literal("get")
								.then(
									Commands.argument("target", ScoreHolderArgument.scoreHolder())
										...
										.then(
											Commands.argument("objective", ObjectiveArgument.objective())
												.executes(c -> getScore(c.getSource(), ScoreHolderArgument.getName(c, "target"), ObjectiveArgument.getObjective(c, "objective")))
										)
								)
						)
						.then(
								...
								.then(
									Commands.argument("targets", ScoreHolderArgument.scoreHolders())
										...
										.then(
											Commands.argument("objective", ObjectiveArgument.objective())
												.executes(
													c -> resetScore(c.getSource(), ScoreHolderArgument.getNamesWithDefaultWildcard(c, "targets"), ObjectiveArgument.getObjective(c, "objective"))
												)
										)
								)
						)
							...
							Commands.literal("enable")
								.then(
									Commands.argument("targets", ScoreHolderArgument.scoreHolders())
										...
										.then(
											Commands.argument("objective", ObjectiveArgument.objective())
												.suggests((c, p) -> suggestTriggers(c.getSource(), ScoreHolderArgument.getNamesWithDefaultWildcard(c, "targets"), p))
												.executes(
													c -> enableTrigger(c.getSource(), ScoreHolderArgument.getNamesWithDefaultWildcard(c, "targets"), ObjectiveArgument.getObjective(c, "objective"))
												)
										)
								)
						)
											...
											Commands.argument("targets", ScoreHolderArgument.scoreHolders())
												...
												.then(
													Commands.argument("objective", ObjectiveArgument.objective())
														...
														.executes(
															c -> setScoreDisplay(
																c.getSource(), ScoreHolderArgument.getNamesWithDefaultWildcard(c, "targets"), ObjectiveArgument.getObjective(c, "objective"), null
															)
														)
												)
										)
												...
												.then(
													addNumberFormats(
														...
														(c, format) -> setScoreNumberFormat(
															c.getSource(), ScoreHolderArgument.getNamesWithDefaultWildcard(c, "targets"), ObjectiveArgument.getObjective(c, "objective"), format
														)
													)
												)
										)
	...
	private static LiteralArgumentBuilder<CommandSourceStack> createRenderTypeModify() {
		...
		for (ObjectiveCriteria.RenderType renderType : ObjectiveCriteria.RenderType.values()) {
			result.then(Commands.literal(renderType.getId()).executes(c -> setRenderType(c.getSource(), ObjectiveArgument.getObjective(c, "objective"), renderType)));
		}
	...
	private static int getScore(final CommandSourceStack source, final ScoreHolder target, final Objective objective) throws CommandSyntaxException {
		...
		source.sendSuccess(
			() -> Component.translatable("commands.scoreboard.players.get.success", target.getFeedbackDisplayName(), score.value(), objective.getFormattedDisplayName()),
			false
		);
		return score.value();
	}
	...
	private static int performOperation(
		...
		if (targets.size() == 1) {
			...
			source.sendSuccess(
				() -> Component.translatable("commands.scoreboard.players.operation.success.multiple", targetObjective.getFormattedDisplayName(), targets.size()), true
			);
		}
	...
	private static int enableTrigger(final CommandSourceStack source, final Collection<ScoreHolder> names, final Objective objective) throws CommandSyntaxException {
		...
		if (names.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.scoreboard.players.enable.success.single", objective.getFormattedDisplayName(), getFirstTargetName(names)), true
			);
		} else {
			source.sendSuccess(
	...
	private static int resetScore(final CommandSourceStack source, final Collection<ScoreHolder> names, final Objective objective) {
		...
		if (names.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.scoreboard.players.reset.specific.single", objective.getFormattedDisplayName(), getFirstTargetName(names)), true
			);
		} else {
			source.sendSuccess(
	...
	private static int setScore(final CommandSourceStack source, final Collection<ScoreHolder> names, final Objective objective, final int value) {
		...
		if (names.size() == 1) {
			source.sendSuccess(
				() -> Component.translatable("commands.scoreboard.players.set.success.single", objective.getFormattedDisplayName(), getFirstTargetName(names), value), true
			);
		} else {
			source.sendSuccess(
	...
	private static int setScoreDisplay(
		...
		if (display == null) {
			if (names.size() == 1) {
				...
				source.sendSuccess(
					() -> Component.translatable("commands.scoreboard.players.display.name.clear.success.multiple", names.size(), objective.getFormattedDisplayName()), true
				);
			}
