											...
											Commands.argument("advancement", ResourceKeyArgument.key(Registries.ADVANCEMENT))
												...
												.then(
													Commands.argument("criterion", StringArgumentType.greedyString())
														.suggests((c, p) -> SharedSuggestionProvider.suggest(ResourceKeyArgument.getAdvancement(c, "advancement").value().criteria().keySet(), p))
														.executes(
															c -> performCriterion(
																c.getSource(),
																EntityArgument.getPlayers(c, "targets"),
	...
	private static int perform(
		...
		if (advancementCount == 0) {
			...
			if (advancements.size() == 1) {
				...
				if (players.size() == 1) {
					source.sendSuccess(
						() -> Component.translatable(action.getKey() + ".one.to.one.success", advancementName, Iterables.getOnlyElement(players).getDisplayName()), true
					);
				} else {
				...
				if (players.size() == 1) {
					source.sendSuccess(
						() -> Component.translatable(action.getKey() + ".many.to.one.success", finalAdvancementCount, Iterables.getOnlyElement(players).getDisplayName()), true
					);
				} else {
					int finalPlayerCount = playerCount;
