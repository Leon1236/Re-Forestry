...
public class RegistrySetBuilder {
		...
		List<HolderLookup.RegistryLookup<?>> lazyFullRegistries = newRegistries.keySet()
			...
			.map(
				registryKey -> this.createLazyFullPatchedRegistries(
					fullPatchedOwner, clonerFactory, (ResourceKey<? extends Registry<? extends Object>>)registryKey, patchOnlyRegistries, fallbackProvider, resultReference
				)
			)
		...
		public <T> BootstrapContext<T> bootstrapContext() {
			return new BootstrapContext<T>() {
				...
				public Holder.Reference<T> register(final ResourceKey<T> key, final T value, final Lifecycle lifecycle) {
					RegistrySetBuilder.RegisteredValue<?> previousValue = BuildState.this.registeredValues.put(key, new RegistrySetBuilder.RegisteredValue(value, lifecycle));
	...
	private record RegistryContents<T>(
		...
		public HolderLookup.RegistryLookup<T> buildAsLookup(final RegistrySetBuilder.UniversalOwner owner) {
			Map<ResourceKey<T>, Holder.Reference<T>> entries = this.values.entrySet().stream().collect(Collectors.toUnmodifiableMap(java.util.Map.Entry::getKey, e -> {
